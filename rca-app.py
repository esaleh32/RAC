...
def app():
    ...
    st.write("""
    # Strength Prediction of Recycled Aggregate Concrete (RAC) Beams App

    **This app predicts the mode of failure, flexural capacity, and shear capacity of RAC beams**
    """)
    st.write ("""
    Press to download the collected Experimental database of RAC beams""")
    import streamlit as st
    import pandas as pd
    ...
    def user_input_features():
        ...
        d = st.sidebar.slider('Beam effective depth, d (mm)',100.0,600.0,250.0)
        ...
        sv=st.sidebar.slider('Stirrups spacing, sv (mm)', 50.0,300.0,150.0)
        data = {'r': r,
                    'dmax': dmax,
                    'b': b,
                    'h': h,
    ...
    df.columns=['r%','d{}{}{} (mm)'.format(get_sub('m'),get_sub('a'),get_sub('x')),'b (mm)','h (mm)','d (mm)','a/d','\u03C1%','\u03C1w %','fy (MPa)','fyw (MPa)','f\'c (MPa)','sv']
    ...
    prediction_shear = load_regs.predict(rr2[:1])
    prediction_fle = (load_regf.predict(rr[:1]))
    cc=[]
    ...
    st.subheader('Shear Strength Prediction, V (kN)')
    ddr=pd.DataFrame(cc)
    ddr.columns=['V (kN)']

    st.write(ddr)

    st.subheader('Flexural Strength Prediction, M (kN.m)')
    mm=[]
    ...
    def vpra(dd):
        ...
        if dd[row]==0:
            ...
            vpra=((1.3+1.6)*(dd[r]**-0.1)*((dd[fc])**0.6)*((dd[dmax]/dd[d])**0.48)*((dd[ad])**-0.91))*dd[b]*dd[d]
        if dd[r]==0:
            beta=0.8*1.25*dd[fc]/(6.89*dd[ro])
            vis=0.85*(np.sqrt(1.25*dd[fc])*(np.sqrt(1+5*beta)-1)/(6*beta))*dd[b]*dd[d]+(dd[row]/1)*dd[fyw]*dd[d]
            vpra=vis
    ...
    st.subheader('Codes Prediction of Shear Strength, V(kN)')
    ...
    st.subheader('Codes Prediction of Flexural Strength, M(kN.m)')
